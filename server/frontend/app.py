...
from frontend.resolver import CapabilityResolver
from schemas.task import BenchmarkConfig, BenchTask, DType, Quant, GPUType
from frontend.presets import MODEL_CONFIGS, PRESET_CONFIGS
from frontend.login import show_auth_screen
...
with benchmark_tab:
    ...
    with col2:
        output_len = st.number_input(
            ...
            help="""Number of output tokens
                ...
                https://docs.vllm.ai/en/stable/cli/bench/serve/#-request-rate
                """,
        )
    ...
    if not use_unbounded:
        request_rate = st.sidebar.number_input(
            ...
            [Request-Rate](https://docs.vllm.ai/en/latest/cli/bench/serve/#-request-rate)
            """,
        )
    else:
        request_rate = None
