...
"""
Handles the entire cluster of GPUs and maintains the global queue of tasks"""

class GPUCluster:
    def __init__(self, gpu_ids: list[int]):
        """initializes available gpu nodes and maintains global queue"""
        self._nodes = {}
    ...
    def get_free_gpus(self, k: int):
        """ returns the k free GPUs (if available) else returns None"""
        free_nodes = []
    ...
    def reserve_gpus(self, nodes: list[GPUNode]):
        """marks status of given input list of GPUs as busy""" 
        for node in nodes:
            node.gpu.status = "busy"

    def release_gpus(self, gpu_ids: list[int]):
        """marks status of given input list of GPUs as free"""
        for gid in gpu_ids:
